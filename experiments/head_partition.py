...
def default_base_dir() -> str:
    return os.environ.get('HEAD_PARTITION_DIR', os.path.join(os.environ.get('HEAD_GEOMETRY_ROOT', "./outputs"), 'agnews', 'head_partition'))
BASE_DIR = default_base_dir()
CACHE_DIR = os.environ.get('HEAD_PARTITION_CACHE_DIR', os.path.join(BASE_DIR, 'cache'))
RESULT_DIR = os.environ.get('HEAD_PARTITION_RESULT_DIR', os.path.join(BASE_DIR, 'results'))
CHECKPOINT_DIR = os.environ.get('HEAD_PARTITION_CHECKPOINT_DIR', os.path.join(BASE_DIR, 'run_checkpoints'))
FINAL_DIR = os.environ.get('HEAD_PARTITION_FINAL_DIR', os.path.join(BASE_DIR, 'final'))

def initialize_output_directories():
    for path in [BASE_DIR, CACHE_DIR, RESULT_DIR, CHECKPOINT_DIR, FINAL_DIR]:
        os.makedirs(path, exist_ok=True)
PHASE = 'agnews_head_partition_control_F6'
SEEDS = list(range(500, 620))
F_VALUES = [6]
EVALUATE_TEST = True
N_PAIRED_SEEDS = len(SEEDS)
ALLOW_INTERIM_INFERENCE = False

def configured_session_hours() -> float:
    return float(os.environ.get('HEAD_PARTITION_MAX_SESSION_HOURS', '0'))
SESSION_DEADLINE_UNIX = float(os.environ.get('HEAD_PARTITION_SESSION_DEADLINE_UNIX', '0'))
SESSION_STOP_BUFFER_SECONDS = 5 * 60

class GracefulSessionStop(RuntimeError):
    pass

def session_stop_due() -> bool:
    return SESSION_DEADLINE_UNIX > 0 and time.time() >= SESSION_DEADLINE_UNIX - SESSION_STOP_BUFFER_SECONDS
...
METADATA_CSV = os.environ.get('HEAD_PARTITION_METADATA_CSV', os.path.join(CACHE_DIR, 'agnews_unique_value_metadata.csv'))
PREPARED_DATA_FILE = os.environ.get('HEAD_PARTITION_PREPARED_DATA_FILE', os.path.join(CACHE_DIR, 'agnews_key_query_minilm.pt'))
N_VALUE_CLASSES = 128
...
D_MODEL = 512
N_LAYERS = 6
WIDE_ARCHITECTURE_ID = 'H8_D64'
PARTITION_CONTROL_ID = 'H16_D32'
ARCHITECTURES = [{'architecture_id': PARTITION_CONTROL_ID, 'n_heads': 16, 'head_dim': 32}, {'architecture_id': WIDE_ARCHITECTURE_ID, 'n_heads': 8, 'head_dim': 64}]
...
GPU_ASSIGNMENTS = {0: [PARTITION_CONTROL_ID], 1: [WIDE_ARCHITECTURE_ID]}
BATCH_SIZE = 256
...
def prepare_data() -> None:
    if os.path.exists(PREPARED_DATA_FILE):
        return
    if load_dataset is None or SentenceTransformer is None:
        raise RuntimeError('Cache preparation dependencies are unavailable. head partition control should normally reuse the exact AG News confirmation cache rather than regenerate it.')
...
def _agnews_confirmation_extract_dir() -> str:
    return os.path.join(os.path.dirname(BASE_DIR), 'head_partition_extracted_agnews_confirmation_source')

def _source_roots() -> list[str]:
    candidates = [os.environ.get('AGNEWS_CONFIRMATION_SOURCE_DIR', os.environ.get('AGNEWS_CONFIRMATION_DIR', os.path.join(os.environ.get('HEAD_GEOMETRY_ROOT', "./outputs"), 'agnews', 'confirmation'))), _agnews_confirmation_extract_dir(), os.environ.get('HEAD_GEOMETRY_INPUT_ROOT', os.path.join(os.environ.get('HEAD_GEOMETRY_ROOT', './outputs'), 'inputs'))]
...
def maybe_extract_agnews_confirmation_archive() -> str | None:
    archive_names = ['AGNEWS_CONFIRMATION_RESULTS.zip']
    archive = None
    for name in archive_names:
        archive = find_external_file(name, preferred_tokens=('agnews/confirmation', 'external_confirmation'))
        if archive is not None:
            break
    if archive is None:
        return None
    destination = _agnews_confirmation_extract_dir()
    decision_path = os.path.join(destination, 'final', 'AGNEWS_CONFIRMATION_DECISION.json')
    if not os.path.exists(decision_path):
        os.makedirs(destination, exist_ok=True)
        shutil.unpack_archive(archive, destination)
    return destination
_MCNEMAR_REJECTION_CACHE: dict[tuple[int, float], list[np.ndarray]] = {}

def _exact_mcnemar_rejection_sets(n_pairs: int, alpha: float) -> list[np.ndarray]:
    key = (int(n_pairs), float(alpha))
    if key in _MCNEMAR_REJECTION_CACHE:
        return _MCNEMAR_REJECTION_CACHE[key]
    rejection_sets: list[np.ndarray] = []
    for discordant in range(n_pairs + 1):
        rejected = []
        if discordant > 0:
            for wide_only in range(discordant + 1):
                p_value = float(stats.binomtest(wide_only, n=discordant, p=0.5, alternative='two-sided').pvalue)
                if p_value < alpha:
                    rejected.append(wide_only)
        rejection_sets.append(np.asarray(rejected, dtype=np.int64))
    _MCNEMAR_REJECTION_CACHE[key] = rejection_sets
    return rejection_sets

def exact_mcnemar_power(n_pairs: int, p_wide_only: float, p_control_only: float, alpha: float=0.05) -> float:
    if p_wide_only < 0 or p_control_only < 0:
        raise ValueError('Discordant probabilities must be nonnegative.')
    p_discordant = p_wide_only + p_control_only
    if p_discordant > 1:
        raise ValueError('Discordant probabilities sum above one.')
    if p_discordant == 0:
        return 0.0
    conditional_wide_probability = p_wide_only / p_discordant
    rejection_sets = _exact_mcnemar_rejection_sets(n_pairs, alpha)
    power = 0.0
    for discordant in range(n_pairs + 1):
        rejected = rejection_sets[discordant]
        if rejected.size == 0:
            continue
        probability_discordant = float(stats.binom.pmf(discordant, n_pairs, p_discordant))
        conditional_rejection = float(stats.binom.pmf(rejected, discordant, conditional_wide_probability).sum())
        power += probability_discordant * conditional_rejection
    return float(power)

def write_power_audit() -> pd.DataFrame:
    scenarios = [('large_asymmetric', 0.3, 0.1), ('moderate_asymmetric', 0.25, 0.1), ('moderate_low_reverse', 0.2, 0.05), ('smaller_asymmetric', 0.2, 0.1), ('small_low_reverse', 0.15, 0.05)]
    sample_sizes = [80, 100, 120, 140, 160, 200]
    rows = []
    for scenario, p10, p01 in scenarios:
        for n_pairs in sample_sizes:
            rows.append({'scenario': scenario, 'n_paired_seeds': n_pairs, 'p_H8_D64_only': p10, 'p_H16_D32_only': p01, 'risk_difference': p10 - p01, 'discordance_probability': p10 + p01, 'exact_two_sided_mcnemar_power': exact_mcnemar_power(n_pairs, p10, p01, PRIMARY_ALPHA)})
    frame = pd.DataFrame(rows)
    frame.to_csv(os.path.join(FINAL_DIR, 'HEAD_PARTITION_SAMPLE_SIZE_POWER_SENSITIVITY.csv'), index=False)
    return frame

def _find_cache_matching_hash(expected_sha256: str) -> str | None:
    candidates = []
    for root in _source_roots():
        for path in glob.glob(os.path.join(root, '**', 'agnews_key_query_minilm.pt'), recursive=True):
...
def verify_agnews_confirmation_lock_and_reuse_cache() -> dict:
    decision_source = find_external_file('AGNEWS_CONFIRMATION_DECISION.json', preferred_tokens=('agnews/confirmation', 'external_confirmation', 'final'))
    audit_source = find_external_file('AGNEWS_CONFIRMATION_SOURCE_LOCK_AUDIT.json', preferred_tokens=('agnews/confirmation', 'external_confirmation', 'final'))
    if decision_source is None or audit_source is None:
        maybe_extract_agnews_confirmation_archive()
        decision_source = find_external_file('AGNEWS_CONFIRMATION_DECISION.json', preferred_tokens=('agnews/confirmation', 'external_confirmation', 'final'))
        audit_source = find_external_file('AGNEWS_CONFIRMATION_SOURCE_LOCK_AUDIT.json', preferred_tokens=('agnews/confirmation', 'external_confirmation', 'final'))
    if decision_source is None:
        raise FileNotFoundError('AGNEWS_CONFIRMATION_DECISION.json was not found. Attach the completed AG News confirmation output folder/ZIP or set AGNEWS_CONFIRMATION_SOURCE_DIR.')
    if audit_source is None:
        raise FileNotFoundError('AGNEWS_CONFIRMATION_SOURCE_LOCK_AUDIT.json was not found. Attach the completed AG News confirmation output folder/ZIP or set AGNEWS_CONFIRMATION_SOURCE_DIR.')
    with open(decision_source, 'r', encoding='utf-8') as file:
        agnews_confirmation_decision = json.load(file)
    with open(audit_source, 'r', encoding='utf-8') as file:
        agnews_confirmation_audit = json.load(file)
    decision = agnews_confirmation_decision.get('decision', {})
    primary = agnews_confirmation_decision.get('primary', {})
    if not bool(decision.get('primary_supported', False)):
        raise RuntimeError("AG News confirmation's frozen primary decision is not supported; head partition control must not present itself as the planned causal follow-up.")
    if int(decision.get('frozen_load', -1)) != SELECTED_F:
        raise RuntimeError('AG News confirmation frozen load does not equal F=6.')
    if int(decision.get('n_paired_seeds', -1)) != 120:
        raise RuntimeError('Unexpected AG News confirmation paired sample size.')
    if not bool(primary.get('confirmatory_primary_supported', False)):
        raise RuntimeError('AG News confirmation primary endpoint record is inconsistent.')
    selected_f = int(agnews_confirmation_audit['selected_F'])
    selected_threshold = float(agnews_confirmation_audit['normalized_binding_threshold'])
    agnews_confirmation_seeds = {int(seed) for seed in agnews_confirmation_audit['confirmatory_seeds']}
    if selected_f != SELECTED_F:
        raise RuntimeError('AG News confirmation source lock does not specify F=6.')
    if not math.isclose(selected_threshold, NORMALIZED_BINDING_REGIME_THRESHOLD, rel_tol=0.0, abs_tol=1e-12):
        raise RuntimeError('AG News confirmation normalized endpoint changed.')
    overlap = sorted(agnews_confirmation_seeds & set(SEEDS))
    if overlap:
        raise RuntimeError(f'head partition control seeds overlap AG News confirmation seeds: {overlap}')
    expected_cache_sha = str(agnews_confirmation_audit['cache_sha256'])
    if os.path.exists(PREPARED_DATA_FILE):
        observed_sha = sha256_file(PREPARED_DATA_FILE)
        if observed_sha != expected_cache_sha:
            raise RuntimeError("Existing head partition control cache does not match AG News confirmation's locked cache SHA-256. Remove the wrong cache before continuing.")
        cache_source = PREPARED_DATA_FILE
    else:
        cache_source = _find_cache_matching_hash(expected_cache_sha)
        if cache_source is None:
            raise FileNotFoundError("No agnews_key_query_minilm.pt matching AG News confirmation's locked SHA-256 was found. Attach AG News confirmation's cache/output ZIP.")
        shutil.copy2(cache_source, PREPARED_DATA_FILE)
    metadata_source = find_external_file('agnews_unique_value_metadata.csv', preferred_tokens=('agnews_confirmation', 'agnews_capacity', 'agnews_transfer'))
    ...
    if payload['encoder_name'] != TEXT_ENCODER_NAME:
        raise RuntimeError('Encoder mismatch against AG News confirmation cache.')
    ...
    shutil.copy2(decision_source, os.path.join(FINAL_DIR, 'FROZEN_AGNEWS_CONFIRMATION_DECISION.json'))
    shutil.copy2(audit_source, os.path.join(FINAL_DIR, 'FROZEN_AGNEWS_CONFIRMATION_SOURCE_LOCK_AUDIT.json'))
    source_audit = {'agnews_confirmation_decision_source': artifact_reference(decision_source), 'agnews_confirmation_decision_sha256': sha256_file(decision_source), 'agnews_confirmation_source_audit_source': artifact_reference(audit_source), 'agnews_confirmation_source_audit_sha256': sha256_file(audit_source), 'agnews_confirmation_primary_supported': True, 'agnews_confirmation_primary_risk_difference': float(primary['paired_risk_difference']), 'agnews_confirmation_primary_mcnemar_p': float(primary['exact_mcnemar_p_value']), 'selected_F': selected_f, 'normalized_binding_threshold': selected_threshold, 'raw_equivalent_validation_threshold': RAW_EQUIVALENT_VALIDATION_THRESHOLD, 'agnews_confirmation_confirmatory_seeds': sorted(agnews_confirmation_seeds), 'head_partition_control_seeds': SEEDS, 'seed_overlap': overlap, 'cache_source': artifact_reference(cache_source), 'cache_destination': artifact_reference(PREPARED_DATA_FILE), 'cache_sha256': sha256_file(PREPARED_DATA_FILE), 'expected_cache_sha256_from_agnews_confirmation': expected_cache_sha, 'dataset_name': payload['dataset_name'], 'encoder_name': payload['encoder_name'], 'key_bank_shape': list(payload['key_embeddings'].shape), 'train_size': int(len(payload['train_indices'])), 'validation_size': int(len(payload['validation_indices'])), 'test_size': int(len(payload['test_indices']))}
    with open(os.path.join(FINAL_DIR, 'HEAD_PARTITION_SOURCE_LOCK_AUDIT.json'), 'w', encoding='utf-8') as file:
        json.dump(source_audit, file, indent=2)
    return source_audit

def state_dict_signature(model: nn.Module) -> list[dict]:
    return [{'name': name, 'shape': list(tensor.shape), 'dtype': str(tensor.dtype)} for name, tensor in model.state_dict().items()]
...
def verify_partition_isolation_and_initialization() -> dict:
    control = ARCHITECTURE_BY_ID[PARTITION_CONTROL_ID]
    wide = ARCHITECTURE_BY_ID[WIDE_ARCHITECTURE_ID]
    if control['inner_dim'] != wide['inner_dim']:
        raise RuntimeError('Total attention inner width is not fixed.')
    if control['d_ff'] != wide['d_ff']:
        raise RuntimeError('FFN width is not fixed.')
    if control['n_heads'] * control['head_dim'] != 512:
        raise RuntimeError('H16_D32 does not have total attention width 512.')
    if wide['n_heads'] * wide['head_dim'] != 512:
        raise RuntimeError('H8_D64 does not have total attention width 512.')
    audit_seed = 20260721
    set_seed(audit_seed)
    model_control = RealTextUniqueValueTransformer(control)
    control_count = count_parameters(model_control)
    control_signature = state_dict_signature(model_control)
    control_hash = state_dict_sha256(model_control)
    del model_control
    gc.collect()
    set_seed(audit_seed)
    model_wide = RealTextUniqueValueTransformer(wide)
    wide_count = count_parameters(model_wide)
    wide_signature = state_dict_signature(model_wide)
    wide_hash = state_dict_sha256(model_wide)
    del model_wide
    gc.collect()
    if control_count != wide_count:
        raise RuntimeError('Parameter count differs across partition control.')
    if control_signature != wide_signature:
        raise RuntimeError('Parameter tensor shapes differ across control arms.')
    if control_hash != wide_hash:
        raise RuntimeError('Same-seed initial parameter tensors are not byte-identical.')
    audit = {'audit_seed': audit_seed, 'wide_architecture_id': WIDE_ARCHITECTURE_ID, 'partition_control_id': PARTITION_CONTROL_ID, 'wide_heads': wide['n_heads'], 'wide_head_dim': wide['head_dim'], 'control_heads': control['n_heads'], 'control_head_dim': control['head_dim'], 'fixed_inner_dim': wide['inner_dim'], 'fixed_d_ff': wide['d_ff'], 'matched_parameter_count': wide_count, 'state_dict_tensor_shapes_identical': True, 'same_seed_parameter_bytes_identical': True, 'initial_state_sha256': wide_hash, 'isolated_difference': 'standard multi-head partition of the same 512-dimensional attention space (8x64 versus 16x32), including the standard head-dimension-dependent attention scale'}
    with open(os.path.join(FINAL_DIR, 'HEAD_PARTITION_PARTITION_ISOLATION_AUDIT.json'), 'w', encoding='utf-8') as file:
        json.dump(audit, file, indent=2)
    return audit

def write_frozen_design_record(source_audit: dict, isolation_audit: dict) -> dict:
    design = {'study': 'head_partition', 'phase': PHASE, 'status': 'prospectively frozen follow-up causal control', 'scientific_question': 'At AG News F=6 with total attention width fixed at 512, does H8_D64 increase associative-regime entry relative to H16_D32?', 'architectures': ARCHITECTURES, 'isolated_comparison': isolation_audit['isolated_difference'], 'agnews_confirmation_decision_sha256': source_audit['agnews_confirmation_decision_sha256'], 'agnews_confirmation_source_audit_sha256': source_audit['agnews_confirmation_source_audit_sha256'], 'cache_sha256': source_audit['cache_sha256'], 'selected_F': SELECTED_F, 'seeds': SEEDS, 'n_paired_seeds': N_PAIRED_SEEDS, 'seed_overlap_with_agnews_confirmation': source_audit['seed_overlap'], 'max_steps': MAX_STEPS, 'primary_endpoint': 'maximum normalized validation binding >= 5/12', 'normalized_binding_formula': '(accuracy - 1/F) / (1 - 1/F)', 'normalized_binding_threshold': NORMALIZED_BINDING_REGIME_THRESHOLD, 'raw_equivalent_validation_threshold': RAW_EQUIVALENT_VALIDATION_THRESHOLD, 'primary_alpha': PRIMARY_ALPHA, 'primary_test': 'exact two-sided paired McNemar test', 'primary_effect': 'paired risk difference P(success|H8_D64) - P(success|H16_D32)', 'primary_decision_rule': ['paired risk difference > 0', 'exact two-sided McNemar p < 0.05', 'paired bootstrap 95% CI lower bound > 0'], 'bootstrap_replicates': BOOTSTRAP_REPLICATES, 'sample_size_rationale': "120 matched pairs fixed before H16_D32 AG News outcomes, matching AG News confirmation's confirmatory scale; power is reported as sensitivity rather than based on an unavailable H16_D32 pilot.", 'no_interim_inference': True, 'no_optional_stopping': True, 'secondary_endpoints': ['held-out test normalized binding >= 5/12', 'maximum normalized validation binding', 'held-out test normalized binding', 'held-out test accuracy', 'maximum logged validation accuracy', 'held-out test accuracy >= 0.80', 'first validation-regime transition step']}
    try:
        design['script_source_sha256'] = sha256_file(os.path.abspath(__file__))
    except Exception:
        design['script_source_sha256'] = None
    with open(os.path.join(FINAL_DIR, 'HEAD_PARTITION_FROZEN_DESIGN.json'), 'w', encoding='utf-8') as file:
...
def save_run_checkpoint(path: str, model: nn.Module, optimizer, scaler, step: int, architecture: dict, n_associations: int, seed: int, learning_curve_rows: list[dict], elapsed_seconds_accumulated: float) -> None:
    atomic_torch_save({'model_state': model.state_dict(), 'optimizer_state': optimizer.state_dict(), 'scaler_state': scaler.state_dict(), 'step': int(step), 'architecture': architecture, 'F': int(n_associations), 'seed': int(seed), 'phase': PHASE, 'rng_state': capture_rng_state(), 'learning_curve_rows': learning_curve_rows, 'elapsed_seconds_accumulated': float(elapsed_seconds_accumulated)}, path)
...
def train_one_run(architecture: dict, n_associations: int, seed: int, key_bank: torch.Tensor, query_bank: torch.Tensor, train_indices: torch.Tensor, validation_indices: torch.Tensor, test_indices: torch.Tensor, device: torch.device) -> dict:
    ...
    if os.path.exists(checkpoint_path):
        ...
        elapsed_seconds_before_resume = float(checkpoint.get('elapsed_seconds_accumulated', max([float(row.get('elapsed_minutes', 0.0)) * 60.0 for row in learning_curve_rows] or [0.0])))
    ...
    for step in range(starting_step, MAX_STEPS + 1):
        ...
        if step % CHECKPOINT_EVERY == 0 or step == MAX_STEPS:
            save_run_checkpoint(path=checkpoint_path, model=model, optimizer=optimizer, scaler=scaler, step=step, architecture=architecture, n_associations=n_associations, seed=seed, learning_curve_rows=learning_curve_rows, elapsed_seconds_accumulated=elapsed_seconds_before_resume + time.time() - run_start)
            if session_stop_due():
                raise GracefulSessionStop('Session deadline reached after safe checkpoint.')
    validation_accuracy = evaluate_fixed_episodes(model=model, key_bank=key_bank, query_bank=query_bank, pool_indices=validation_indices, n_associations=n_associations, batch_size=VALIDATION_BATCH_SIZE, number_of_batches=VALIDATION_BATCHES, episode_seed=VALIDATION_EPISODE_SEED_BASE + 1000 * n_associations, device=device, return_predictions=False)
    ...
    elapsed_seconds = elapsed_seconds_before_resume + time.time() - run_start
...
def run_architecture(architecture: dict, key_bank: torch.Tensor, query_bank: torch.Tensor, train_indices: torch.Tensor, validation_indices: torch.Tensor, test_indices: torch.Tensor, device: torch.device) -> None:
    ...
    del temporary_model
    config = {'phase': PHASE, 'scientific_question': 'prospectively frozen AG News fixed-attention-width head-partition control at F=6', 'F_values': F_VALUES, 'seeds': SEEDS, **architecture, 'parameter_count': parameter_count, 'd_model': D_MODEL, 'n_layers': N_LAYERS, 'max_steps': MAX_STEPS, 'primary_endpoint': 'maximum_logged_validation_binding >= 5/12', 'normalized_regime_success_threshold': NORMALIZED_BINDING_REGIME_THRESHOLD, 'raw_equivalent_validation_threshold': RAW_EQUIVALENT_VALIDATION_THRESHOLD, 'secondary_endpoint': 'held-out test normalized binding >= 5/12', 'high_test_success_threshold': HIGH_TEST_SUCCESS_THRESHOLD}
    ...
    for n_associations in F_VALUES:
        ...
        for seed in SEEDS:
            if session_stop_due():
                raise GracefulSessionStop('Session deadline reached before starting a new run.')
...
def paired_continuous_contrast(paired: pd.DataFrame, outcome: str, rng: np.random.Generator) -> dict:
    ...
    narrow = paired[f'{outcome}_H16_D32'].to_numpy(dtype=float)
    differences = wide - narrow
    indices = rng.integers(0, len(differences), size=(BOOTSTRAP_REPLICATES, len(differences)))
    bootstrap = differences[indices].mean(axis=1)
    low, high = np.quantile(bootstrap, [0.025, 0.975])
    return {'outcome': outcome, 'paired_seeds': int(len(differences)), 'mean_H8_D64': float(wide.mean()), 'mean_H16_D32': float(narrow.mean()), 'mean_difference_H8_D64_minus_H16_D32': float(differences.mean()), 'median_difference_H8_D64_minus_H16_D32': float(np.median(differences)), 'bootstrap_ci_low': float(low), 'bootstrap_ci_high': float(high), 'exact_sign_flip_p_value': float(exact_sign_flip_p_value(differences)), 'wins_H8_D64': int(np.sum(differences > 0)), 'ties': int(np.sum(differences == 0)), 'losses_H8_D64': int(np.sum(differences < 0))}
...
def binary_endpoint_summary(paired: pd.DataFrame, column: str, endpoint_name: str, rng: np.random.Generator) -> dict:
    ...
    narrow = paired[f'{column}_H16_D32'].astype(bool).to_numpy()
    ...
    return {'endpoint': endpoint_name, 'n_paired_seeds': int(len(paired)), 'n_success_H8_D64': wide_count, 'p_success_H8_D64': float(wide.mean()), 'p_success_H8_D64_ci_low': wide_ci[0], 'p_success_H8_D64_ci_high': wide_ci[1], 'n_success_H16_D32': narrow_count, 'p_success_H16_D32': float(narrow.mean()), 'p_success_H16_D32_ci_low': narrow_ci[0], 'p_success_H16_D32_ci_high': narrow_ci[1], 'paired_risk_difference': rd, 'bootstrap_ci_low': low, 'bootstrap_ci_high': high, **mcnemar}
...
def save_confirmation_figures(merged: pd.DataFrame, paired: pd.DataFrame, primary: dict, cumulative: pd.DataFrame) -> None:
    ...
    labels = ['H16_D32', 'H8_D64']
    probabilities = [primary['p_success_H16_D32'], primary['p_success_H8_D64']]
    lower = [probabilities[0] - primary['p_success_H16_D32_ci_low'], probabilities[1] - primary['p_success_H8_D64_ci_low']]
    upper = [primary['p_success_H16_D32_ci_high'] - probabilities[0], primary['p_success_H8_D64_ci_high'] - probabilities[1]]
    ...
    axis.set_title('AG News head-partition control, F=6, 120 new matched seeds')
    axis.grid(axis='y', alpha=0.25)
    fig.tight_layout()
    for extension in ['pdf', 'png']:
        fig.savefig(os.path.join(figure_dir, f'head_partition_primary_success_probability.{extension}'), dpi=350 if extension == 'png' else None, bbox_inches='tight')
    plt.close(fig)
    x_narrow = paired['maximum_logged_validation_binding_H16_D32'].to_numpy(dtype=float)
    ...
    axis.set_xlabel('H16_D32 maximum validation binding')
    axis.set_ylabel('H8_D64 maximum validation binding')
    axis.set_title('Seed-matched fixed-width partition outcomes')
    axis.grid(alpha=0.25)
    fig.tight_layout()
    for extension in ['pdf', 'png']:
        fig.savefig(os.path.join(figure_dir, f'head_partition_paired_maximum_binding.{extension}'), dpi=350 if extension == 'png' else None, bbox_inches='tight')
    plt.close(fig)
    fig, axis = plt.subplots(figsize=(6.4, 4.4))
    for architecture_id in ['H16_D32', 'H8_D64']:
    ...
    fig.tight_layout()
    for extension in ['pdf', 'png']:
        fig.savefig(os.path.join(figure_dir, f'head_partition_cumulative_regime_entry.{extension}'), dpi=350 if extension == 'png' else None, bbox_inches='tight')
    plt.close(fig)
    differences = paired['test_binding_score_H8_D64'].to_numpy(dtype=float) - paired['test_binding_score_H16_D32'].to_numpy(dtype=float)
    ...
    axis.set_xlabel('Paired held-out binding difference (H8_D64 - H16_D32)')
    axis.set_ylabel('Number of seed pairs')
    axis.legend()
    axis.grid(axis='y', alpha=0.25)
    fig.tight_layout()
    for extension in ['pdf', 'png']:
        fig.savefig(os.path.join(figure_dir, f'head_partition_paired_test_binding_difference.{extension}'), dpi=350 if extension == 'png' else None, bbox_inches='tight')
    plt.close(fig)

def control_sequence_synthesis(current_primary: dict) -> pd.DataFrame:
    frozen_decision = os.path.join(FINAL_DIR, 'FROZEN_AGNEWS_CONFIRMATION_DECISION.json')
    rows = []
    if os.path.exists(frozen_decision):
        with open(frozen_decision, 'r', encoding='utf-8') as file:
            old_payload = json.load(file)
        old = old_payload['primary']
        rows.append({'study': 'AG News confirmation: parameter-allocation comparison', 'comparison': 'H8_D64 - H8_D32', 'load_F': 6, 'n_paired_seeds': int(old['n_paired_seeds']), 'p_success_first': float(old['p_success_H8_D64']), 'p_success_second': float(old['p_success_H8_D32']), 'paired_risk_difference': float(old['paired_risk_difference']), 'ci_low': float(old['bootstrap_ci_low']), 'ci_high': float(old['bootstrap_ci_high']), 'exact_mcnemar_p_value': float(old['exact_mcnemar_p_value']), 'status': 'independent external confirmation'})
    rows.append({'study': 'head partition control: fixed-attention-width partition control', 'comparison': 'H8_D64 - H16_D32', 'load_F': SELECTED_F, 'n_paired_seeds': current_primary['n_paired_seeds'], 'p_success_first': current_primary['p_success_H8_D64'], 'p_success_second': current_primary['p_success_H16_D32'], 'paired_risk_difference': current_primary['paired_risk_difference'], 'ci_low': current_primary['bootstrap_ci_low'], 'ci_high': current_primary['bootstrap_ci_high'], 'exact_mcnemar_p_value': current_primary['exact_mcnemar_p_value'], 'status': 'prospectively frozen causal control'})
    frame = pd.DataFrame(rows)
    frame.to_csv(os.path.join(FINAL_DIR, 'HEAD_PARTITION_CONTROL_SEQUENCE_SYNTHESIS.csv'), index=False)
    if len(frame) > 1:
        figure_dir = os.path.join(FINAL_DIR, 'figures')
        os.makedirs(figure_dir, exist_ok=True)
        ...
        axis.set_xlabel('Paired regime-entry risk difference')
        axis.grid(axis='x', alpha=0.25)
        fig.tight_layout()
        for extension in ['pdf', 'png']:
            fig.savefig(os.path.join(figure_dir, f'head_partition_control_sequence_forest.{extension}'), dpi=350 if extension == 'png' else None, bbox_inches='tight')
        plt.close(fig)
    return frame
...
def merge_results() -> None:
    ...
    if set(merged['F'].astype(int)) != {SELECTED_F}:
        raise RuntimeError('Completed load differs from the frozen F=6 source lock.')
    all_runs_path = os.path.join(FINAL_DIR, 'HEAD_PARTITION_ALL_RUNS.csv')
    ...
    summary.to_csv(os.path.join(FINAL_DIR, 'HEAD_PARTITION_ARCHITECTURE_SUMMARY.csv'), index=False)
    wide = merged[merged['architecture_id'] == 'H8_D64'].copy()
    narrow = merged[merged['architecture_id'] == 'H16_D32'].copy()
    paired = wide.merge(narrow, on=['seed', 'F'], suffixes=('_H8_D64', '_H16_D32'), validate='one_to_one').sort_values('seed')
    ...
    pd.DataFrame([primary]).to_csv(os.path.join(FINAL_DIR, 'HEAD_PARTITION_PRIMARY_ENDPOINT.csv'), index=False)
    ...
    pd.DataFrame([heldout_binary]).to_csv(os.path.join(FINAL_DIR, 'HEAD_PARTITION_HELDOUT_BINARY_ENDPOINT.csv'), index=False)
    ...
    pd.DataFrame([stringent_binary]).to_csv(os.path.join(FINAL_DIR, 'HEAD_PARTITION_STRINGENT_TEST_ENDPOINT.csv'), index=False)
    ...
    continuous.to_csv(os.path.join(FINAL_DIR, 'HEAD_PARTITION_CONTINUOUS_OUTCOMES.csv'), index=False)
    transition_rows = []
    ...
    transitions.to_csv(os.path.join(FINAL_DIR, 'HEAD_PARTITION_TRANSITIONS.csv'), index=False)
    cumulative = cumulative_transition_frame(transitions)
    cumulative.to_csv(os.path.join(FINAL_DIR, 'HEAD_PARTITION_CUMULATIVE_REGIME_ENTRY.csv'), index=False)
    save_confirmation_figures(merged, paired, primary, cumulative)
    control_sequence = control_sequence_synthesis(primary)
    decision = {'primary_supported': bool(primary['confirmatory_primary_supported']), 'frozen_load': SELECTED_F, 'n_paired_seeds': N_PAIRED_SEEDS, 'interpretation': 'Fixed-attention-width head-partition control supported: at AG News F=6, H8_D64 significantly increases associative-regime entry relative to H16_D32 while total attention width, FFN width, parameter count, parameter shapes, and same-seed initialization are held fixed.' if primary['confirmatory_primary_supported'] else 'The fixed-attention-width head-partition control was not supported under the frozen paired binary rule. Report the result without changing the load, threshold, seed cohort, or sample size.', 'prohibited_post_result_actions': ['Do not extend the seed cohort after inspecting head partition control.', 'Do not change F=6.', 'Do not change the normalized threshold 5/12.', 'Do not replace the exact two-sided McNemar primary test.', 'Do not relabel a secondary endpoint as primary.']}
    with open(os.path.join(FINAL_DIR, 'HEAD_PARTITION_DECISION.json'), 'w', encoding='utf-8') as file:
    ...
    print(control_sequence.to_string(index=False), flush=True)
    print(json.dumps(decision, indent=2), flush=True)

def package_outputs() -> str:
    archive_base = os.path.join(os.path.dirname(BASE_DIR), 'HEAD_PARTITION_RESULTS')
...
def verify_frozen_design_constants() -> None:
    ...
    if len(SEEDS) != 120 or SEEDS != list(range(500, 620)):
        raise RuntimeError('head partition control control seeds were modified.')
    ...
    ids = {architecture['architecture_id'] for architecture in ARCHITECTURES}
    if ids != {WIDE_ARCHITECTURE_ID, PARTITION_CONTROL_ID}:
        raise RuntimeError('head partition control architecture IDs were modified.')

def progress_status() -> tuple[pd.DataFrame, bool]:
    rows = []
    complete = True
    for architecture in ARCHITECTURES:
        architecture_id = architecture['architecture_id']
        result_pkl, _, _ = result_paths(architecture_id)
        completed = 0
        if os.path.exists(result_pkl):
            with open(result_pkl, 'rb') as file:
                payload = pickle.load(file)
            completed = len(payload.get(SELECTED_F, {}))
        expected = len(SEEDS)
        rows.append({'architecture_id': architecture_id, 'completed_runs': completed, 'expected_runs': expected, 'remaining_runs': expected - completed, 'fraction_complete': completed / expected})
        complete = complete and completed == expected
    frame = pd.DataFrame(rows)
    frame.to_csv(os.path.join(FINAL_DIR, 'HEAD_PARTITION_PROGRESS.csv'), index=False)
    return (frame, complete)

def parent_main() -> None:
    verify_frozen_design_constants()
    source_audit = verify_agnews_confirmation_lock_and_reuse_cache()
    isolation_audit = verify_partition_isolation_and_initialization()
    frozen_design = write_frozen_design_record(source_audit, isolation_audit)
    write_power_audit()
    ...
    deadline = time.time() + session_hours * 3600.0 if session_hours > 0 else 0.0
    if deadline > 0:
        pass
    processes = []
    for physical_gpu, architecture_ids in assignments.items():
        ...
        environment['HEAD_PARTITION_SESSION_DEADLINE_UNIX'] = str(deadline)
        environment['HEAD_PARTITION_DIR'] = BASE_DIR
    ...
    progress, complete = progress_status()
...
if __name__ == '__main__':
    ...
    if arguments.worker:
        ...
        except GracefulSessionStop as error:
            sys.exit(0)
    else:
        parent_main()
