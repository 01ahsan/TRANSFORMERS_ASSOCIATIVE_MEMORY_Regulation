"""Factorial head geometry experiment."""
...
def parse_arguments():
    parser = argparse.ArgumentParser(description='Factorial head geometry experiment.')
...
BASE_DIR = os.environ.get("HEAD_FACTORIAL_DIR", os.path.join(OUTPUT_ROOT, "wikitext", "head_factorial"))
...
ARCHITECTURES = [
    ...
    {"architecture_id": "H16_D32", "n_heads": 16, "head_dim": 32},
    {"architecture_id": "H16_D64", "n_heads": 16, "head_dim": 64},
]
...
def train_one_run(
    ...
    result = {
        "phase": PHASE,
        "experiment": "fully_active_head_count_by_head_width_factorial",
        "architecture_id": architecture_id,
        ...
        "success": bool(
            test_accuracy >= SUCCESS_THRESHOLD
        ),
...
def run_architecture(
    ...
    config = {
        ...
        "scientific_question": (
            ...
            "head count and fully active per-head width"
        ),
        "F_values": F_VALUES,
        "seeds": SEEDS,
        **architecture,
...
def paired_contrast(
    ...
    a = merged[
        merged["architecture_id"] == architecture_a
    ][["seed", "test_accuracy"]].rename(
        columns={"test_accuracy": "value_a"}
    )
    b = merged[
        merged["architecture_id"] == architecture_b
    ][["seed", "test_accuracy"]].rename(
        columns={"test_accuracy": "value_b"}
    )
    paired = a.merge(b, on="seed").sort_values("seed")
    differences = (
        paired["value_a"].to_numpy(dtype=float)
        - paired["value_b"].to_numpy(dtype=float)
    )
    ...
    return {
        ...
        "losses_a": int(np.sum(differences < 0)),
    }
...
def compute_factorial_effects(
    ...
    for column in required:
        if column not in pivot.columns:
            raise RuntimeError(
                f"Missing factorial cell: {column}"
            )

    width_effect_h8 = (
        pivot[(8, 64)] - pivot[(8, 32)]
    ).to_numpy(dtype=float)
    width_effect_h16 = (
        pivot[(16, 64)] - pivot[(16, 32)]
    ).to_numpy(dtype=float)
    head_effect_d32 = (
        pivot[(16, 32)] - pivot[(8, 32)]
    ).to_numpy(dtype=float)
    head_effect_d64 = (
        pivot[(16, 64)] - pivot[(8, 64)]
    ).to_numpy(dtype=float)

    width_main = 0.5 * (
        width_effect_h8 + width_effect_h16
    )
    head_main = 0.5 * (
        head_effect_d32 + head_effect_d64
    )
    interaction = (
        width_effect_h16 - width_effect_h8
    )

    effects = {
        "head_width_main_effect_64_minus_32": width_main,
        "head_count_main_effect_16_minus_8": head_main,
        "interaction_difference_in_width_effects": interaction,
        "simple_width_effect_at_8_heads": width_effect_h8,
        "simple_width_effect_at_16_heads": width_effect_h16,
        "simple_head_effect_at_width_32": head_effect_d32,
        "simple_head_effect_at_width_64": head_effect_d64,
    }
        ...
        rows.append(
            {
                ...
                "bootstrap_ci_high": float(ci_high),
                "exact_sign_flip_p_value": float(
                    exact_sign_flip_p_value(values)
                ),
                "positive_seeds": int(
                    np.sum(values > 0)
                ),
...
def merge_results() -> None:
    ...
    if merged.duplicated(
        ...
        raise RuntimeError(
            "Duplicate architecture/F/seed rows found."
        )

    parameter_counts = merged[
        "parameter_count"
    ].unique()
    if len(parameter_counts) != 1:
        raise RuntimeError(
            "Parameter counts are not matched: "
    ...
    summary = merged.groupby(
        ...
        mean_final_gradient_norm=(
            "final_gradient_norm_before_clipping",
            "mean",
        ),
        mean_attention_entropy=(
            "final_attention_entropy", "mean"
        ),
        mean_attention_logit_std=(
            "final_attention_logit_std", "mean"
        ),
    ).reset_index()

    summary_path = os.path.join(
        FINAL_DIR,
        "confirmatory_POINT_SUMMARY.csv",
    )
    ...
    contrast_specs = [
        (
            "H8_D64",
            "H8_D32",
            "width_64_minus_32_at_8_heads",
        ),
        (
            "H16_D64",
            "H16_D32",
            "width_64_minus_32_at_16_heads",
        ),
        (
            "H16_D32",
            "H8_D32",
            "heads_16_minus_8_at_width_32",
        ),
        (
            "H16_D64",
            "H8_D64",
            "heads_16_minus_8_at_width_64",
        ),
        (
            "H16_D64",
            "H8_D32",
            "largest_inner_minus_smallest_inner",
        ),
    ]
    ...
    contrasts_path = os.path.join(
        FINAL_DIR,
        "confirmatory_PAIRED_CONTRASTS.csv",
    )
    contrasts.to_csv(contrasts_path, index=False)

    factorial = compute_factorial_effects(
        merged,
        rng=np.random.default_rng(20260720),
    )
    factorial_path = os.path.join(
        FINAL_DIR,
        "confirmatory_FACTORIAL_EFFECTS.csv",
    )
...
def parent_main() -> None:
    ...
    print(
        f"[MATCHED PARAMETER COUNT] "
        f"{matched_parameter_count:,}",
        flush=True,
    )
    ...
    if gpu_count < 1:
        raise RuntimeError(
            "At least one CUDA GPU is required."
        )
