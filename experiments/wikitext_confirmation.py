"""WikiText probability of success confirmation."""
...
def parse_arguments():
    parser = argparse.ArgumentParser(description='WikiText probability of success confirmation.')
...
BASE_DIR = os.environ.get("WIKITEXT_CONFIRMATION_DIR", os.path.join(OUTPUT_ROOT, "wikitext", "confirmation"))
...
PHASE = "confirmatory_replication"
...
REGIME_SUCCESS_THRESHOLD = 0.50


HIGH_TEST_SUCCESS_THRESHOLD = 0.80
...
ARCHITECTURES = [
    {"architecture_id": "H8_D32", "n_heads": 8, "head_dim": 32},
    {"architecture_id": "H8_D64", "n_heads": 8, "head_dim": 64},
]
...
for architecture in ARCHITECTURES:
    architecture["inner_dim"] = (
        architecture["n_heads"] * architecture["head_dim"]
    )
    architecture["d_ff"] = (
        BASE_D_FF
        + 2 * (TARGET_INNER_DIM - architecture["inner_dim"])
    )
...
class FullyActiveMultiHeadSelfAttention(nn.Module):
    """Multi-head self-attention with independent fully active inner width."""
    ...
    def forward(
        ...
        if collect_diagnostics:
            with torch.no_grad():
                ...
                entropy = -(
                    attention.clamp_min(1e-9)
                    * attention.clamp_min(1e-9).log()
                ).sum(dim=-1)

                self.last_diagnostics = {
                    "q_rms": float(q.float().pow(2).mean().sqrt().item()),
                    "k_rms": float(k.float().pow(2).mean().sqrt().item()),
                    "v_rms": float(v.float().pow(2).mean().sqrt().item()),
                    "attention_logit_std": (
                        float(finite_logits.float().std().item())
                        if finite_logits.numel() > 1
                        else 0.0
                    ),
                    "attention_entropy": float(
                        entropy.float().mean().item()
                    ),
                    "attention_max_probability": float(
                        attention.float().amax(dim=-1).mean().item()
                    ),
                }
...
class TransformerBlock(nn.Module):
    def __init__(
        ...
        self.attention = FullyActiveMultiHeadSelfAttention(
            d_model=d_model,
            n_heads=n_heads,
            head_dim=head_dim,
        )
...
class RealTextUniqueValueTransformer(nn.Module):
    def __init__(self, architecture: dict):
        ...
        self.real_text_projection = nn.Linear(
            D_INPUT, D_MODEL, bias=False
        )
        self.value_embeddings = nn.Embedding(
            N_VALUE_CLASSES, D_MODEL
        )
    ...
    def forward(
        ...
        positions = torch.arange(
            sequence_length, device=sequence.device
        )
    ...
    def aggregate_attention_diagnostics(self) -> dict[str, float]:
        ...
        for key in keys:
            values = [
                block.attention.last_diagnostics[key]
                for block in self.blocks
                if key in block.attention.last_diagnostics
            ]
            output[key] = (
...
def verify_parameter_matching() -> int:
    ...
    print("[PARAMETER AUDIT]", flush=True)
    for architecture_id, count in counts.items():
        architecture = ARCHITECTURE_BY_ID[architecture_id]
        print(
            f"{architecture_id}: heads={architecture['n_heads']}, "
            f"head_dim={architecture['head_dim']}, "
            f"inner_dim={architecture['inner_dim']}, "
            f"d_ff={architecture['d_ff']}, "
            f"parameters={count:,}",
            flush=True,
        )
    ...
    if len(unique_counts) != 1:
        raise RuntimeError(
            f"Parameter matching failed: {counts}"
        )
...
def collect_diagnostics(
    model: RealTextUniqueValueTransformer,
    key_bank: torch.Tensor,
    query_bank: torch.Tensor,
    ...
    keys, value_ids, queries, _ = generate_episode_batch(
        key_bank=key_bank,
        query_bank=query_bank,
        pool_indices=validation_indices,
        n_associations=n_associations,
        batch_size=min(VALIDATION_BATCH_SIZE, 256),
        device=device,
        generator=generator,
    )
...
def run_checkpoint_path(
    ...
    return os.path.join(
        CHECKPOINT_DIR,
        f"{PHASE}_{architecture_id}_F{n_associations}_seed{seed}.pt",
    )
...
def learning_curve_path(
    ...
    return os.path.join(
        directory,
        f"{PHASE}_{architecture_id}_F{n_associations}_seed{seed}.csv",
    )
...
def train_one_run(
    ...
    curve_path = learning_curve_path(
        architecture_id,
        n_associations,
        seed,
    )
    ...
    if os.path.exists(checkpoint_path):
        ...
        learning_curve_rows = list(
            checkpoint.get("learning_curve_rows", [])
        )
        restore_rng_state(checkpoint.get("rng_state"))

        print(
            f"[RESUME RUN] architecture={architecture_id}, "
    ...
    for step in range(starting_step, MAX_STEPS + 1):
        ...
        if should_log:
            ...
            print(
                ...
                f"heads={architecture['n_heads']:>2} | "
                f"head_dim={architecture['head_dim']:>2} | "
                f"inner={architecture['inner_dim']:>4} | "
                f"d_ff={architecture['d_ff']:>4} | "
                f"F={n_associations} | seed={seed:>2} | "
                f"step={step:>5}/{MAX_STEPS} | "
                f"loss={last_loss:.5f} | "
                f"val_acc={validation_accuracy:.4f} | "
                f"binding={row['validation_binding_score']:.4f} | "
                f"grad={last_gradient_norm:.4f} | "
                f"entropy={diagnostics['attention_entropy']:.4f} | "
                f"logit_std={diagnostics['attention_logit_std']:.4f} | "
                f"time={row['elapsed_minutes']:.1f} min",
                flush=True,
            )
    ...
    final_diagnostics = collect_diagnostics(
        model=model,
        key_bank=key_bank,
        query_bank=query_bank,
        validation_indices=validation_indices,
    ...
    result = {
        "phase": PHASE,
        "experiment": "confirmatory_probability_of_success_replication",
        "architecture_id": architecture_id,
        ...
        "maximum_logged_validation_accuracy": float(
            max(
                row["validation_accuracy"]
                for row in learning_curve_rows
            )
        ),
        "entered_regime_050": bool(
            max(
                row["validation_accuracy"]
                for row in learning_curve_rows
            )
        ...
        "high_test_success_080": bool(
            test_accuracy >= HIGH_TEST_SUCCESS_THRESHOLD
        ),
        ...
        "global_chance_accuracy": float(
            GLOBAL_CHANCE_ACCURACY
        ),
        "d_input": D_INPUT,
        "d_model": D_MODEL,
        "n_layers": N_LAYERS,
...
def run_architecture(
    ...
    result_pkl, result_csv, config_json = result_paths(
        architecture_id
    )

    temporary_model = RealTextUniqueValueTransformer(
        architecture
    )
    ...
    config = {
        "phase": PHASE,
        "scientific_question": (
            "confirmatory probability-of-success effect of fully active "
            "head width at eight attention heads"
        ),
        "F_values": F_VALUES,
        "seeds": SEEDS,
        **architecture,
        ...
        "n_layers": N_LAYERS,
        "max_steps": MAX_STEPS,
        "primary_endpoint": "maximum_logged_validation_accuracy >= 0.50",
        "regime_success_threshold": REGIME_SUCCESS_THRESHOLD,
        "secondary_endpoint": "test_accuracy >= 0.80",
        "high_test_success_threshold": HIGH_TEST_SUCCESS_THRESHOLD,
    }
    ...
    for n_associations in F_VALUES:
        ...
        for seed in SEEDS:
            if int(seed) in results[int(n_associations)]:
                print(
                    f"[SKIP COMPLETE] architecture={architecture_id}, "
                    f"F={n_associations}, seed={seed}",
                    flush=True,
                )
                continue
            ...
            print(
                f"TRAINING | architecture={architecture_id} | "
                f"F={n_associations} | seed={seed}",
                flush=True,
            )
            ...
            print(
                f"RESULT | architecture={architecture_id} | "
                f"heads={architecture['n_heads']} | "
                f"head_dim={architecture['head_dim']} | "
                f"F={n_associations} | seed={seed} | "
...
def worker_main(architecture_ids_text: str) -> None:
    ...
    architecture_ids = [
        value.strip()
        for value in architecture_ids_text.split(",")
        if value.strip()
    ]
    unknown = [
        architecture_id
        for architecture_id in architecture_ids
        if architecture_id not in ARCHITECTURE_BY_ID
    ]
    if unknown:
        raise ValueError(
            f"Unknown architecture IDs: {unknown}"
        )
    ...
    key_bank = payload["key_embeddings"].to(
        device
    ).contiguous()
    query_bank = payload["query_embeddings"].to(
        device
    ).contiguous()
...
def exact_mcnemar_test(
    success_wide: np.ndarray,
    success_narrow: np.ndarray,
) -> dict:
    ...
    return {
        "wide_only_success": wide_only,
        "narrow_only_success": narrow_only,
        "both_success": both,
...
def paired_risk_difference_bootstrap(
    ...
    indices = rng.integers(
        ...
        size=(BOOTSTRAP_REPLICATES, len(paired_differences)),
    )
...
def exact_sign_flip_p_value(
    ...
    return float(
        (1 + np.sum(null_statistics >= observed - 1e-15))
...
def paired_continuous_contrast(
    ...
    return {
        ...
        "mean_H8_D64": float(wide.mean()),
        "mean_H8_D32": float(narrow.mean()),
        "mean_difference_H8_D64_minus_H8_D32": float(
            differences.mean()
        ),
        "median_difference_H8_D64_minus_H8_D32": float(
        ...
        "wins_H8_D64": int(np.sum(differences > 0)),
        "ties": int(np.sum(differences == 0)),
        "losses_H8_D64": int(np.sum(differences < 0)),
    }


def transition_step_from_curve(
    curve_path: str,
    threshold: float = REGIME_SUCCESS_THRESHOLD,
) -> float:
    ...
    matching = curve[
        curve["validation_accuracy"] >= threshold
    ]
    if matching.empty:
        return float("nan")
...
def merge_results() -> None:
    csv_files = sorted(
        glob.glob(os.path.join(RESULT_DIR, "*.csv"))
    )
    frames = [
        pd.read_csv(path)
        for path in csv_files
        if os.path.getsize(path) > 0
        and "learning_curves" not in path
    ]
    if not frames:
    ...
    merged = pd.concat(
        ...
        ["architecture_id", "seed"]
    ).reset_index(drop=True)

    expected_rows = len(ARCHITECTURES) * len(F_VALUES) * len(SEEDS)
    if len(merged) != expected_rows:
        raise RuntimeError(
            f"Incomplete result grid: found {len(merged)}, "
            f"expected {expected_rows}."
        )

    duplicate_count = int(
        merged.duplicated(
            ["architecture_id", "F", "seed"]
        ).sum()
    )
    if duplicate_count:
        raise RuntimeError(
            f"Duplicate architecture/F/seed rows: {duplicate_count}"
        )

    all_runs_path = os.path.join(
        FINAL_DIR,
        "confirmatory_replication_ALL_RUNS.csv",
    )
    merged.to_csv(all_runs_path, index=False)

    summary = (
        merged.groupby("architecture_id")
        .agg(
            n_runs=("seed", "count"),
            mean_test_accuracy=("test_accuracy", "mean"),
            median_test_accuracy=("test_accuracy", "median"),
            std_test_accuracy=("test_accuracy", "std"),
            mean_max_validation_accuracy=(
                "maximum_logged_validation_accuracy",
                "mean",
            ),
            n_regime_success=("entered_regime_050", "sum"),
            p_regime_success=("entered_regime_050", "mean"),
            n_high_test_success=("high_test_success_080", "sum"),
            p_high_test_success=("high_test_success_080", "mean"),
            mean_test_binding=("test_binding_score", "mean"),
            mean_elapsed_minutes=(
                "elapsed_seconds",
                lambda values: float(values.mean() / 60.0),
            ),
        )
        .reset_index()
    )
    summary_path = os.path.join(
        FINAL_DIR,
        "confirmatory_replication_SUMMARY.csv",
    )
    summary.to_csv(summary_path, index=False)

    wide = merged[
        merged["architecture_id"] == "H8_D64"
    ].copy()
    narrow = merged[
        merged["architecture_id"] == "H8_D32"
    ].copy()

    paired = wide.merge(
        narrow,
        on=["seed", "F"],
        suffixes=("_H8_D64", "_H8_D32"),
        validate="one_to_one",
    ).sort_values("seed")

    if len(paired) != len(SEEDS):
        raise RuntimeError(
            f"Expected {len(SEEDS)} paired seeds, found {len(paired)}."
        )

    rng = np.random.default_rng(ANALYSIS_RANDOM_SEED)

    wide_success = paired[
        "entered_regime_050_H8_D64"
    ].astype(bool).to_numpy()
    narrow_success = paired[
        "entered_regime_050_H8_D32"
    ].astype(bool).to_numpy()

    risk_difference, rd_low, rd_high = (
    ...
    primary = {
        "primary_endpoint": (
            "maximum logged validation accuracy >= 0.50"
        ),
        "n_paired_seeds": int(len(paired)),
        "p_success_H8_D64": float(wide_success.mean()),
        "p_success_H8_D32": float(narrow_success.mean()),
        "paired_risk_difference": risk_difference,
        "bootstrap_ci_low": rd_low,
        "bootstrap_ci_high": rd_high,
        **mcnemar,
        "alpha": PRIMARY_ALPHA,
    }


    primary["confirmatory_primary_supported"] = bool(
        primary["paired_risk_difference"] > 0
        and primary["exact_mcnemar_p_value"] < PRIMARY_ALPHA
        and primary["bootstrap_ci_low"] > 0
    )

    primary_frame = pd.DataFrame([primary])
    primary_path = os.path.join(
        FINAL_DIR,
        "confirmatory_replication_PRIMARY_ENDPOINT.csv",
    )
    ...
    continuous_path = os.path.join(
        FINAL_DIR,
        "confirmatory_replication_CONTINUOUS_OUTCOMES.csv",
    )
    continuous.to_csv(continuous_path, index=False)

    wide_high = paired[
        "high_test_success_080_H8_D64"
    ].astype(bool).to_numpy()
    narrow_high = paired[
        "high_test_success_080_H8_D32"
    ].astype(bool).to_numpy()
    high_rd, high_low, high_high = (
        paired_risk_difference_bootstrap(
            wide_high,
            narrow_high,
            rng,
    ...
    secondary_binary = {
        "secondary_endpoint": "held-out test accuracy >= 0.80",
        "n_paired_seeds": int(len(paired)),
        "p_success_H8_D64": float(wide_high.mean()),
        "p_success_H8_D32": float(narrow_high.mean()),
        "paired_risk_difference": high_rd,
        "bootstrap_ci_low": high_low,
        "bootstrap_ci_high": high_high,
        **exact_mcnemar_test(wide_high, narrow_high),
    }
    secondary_binary_path = os.path.join(
        FINAL_DIR,
        "confirmatory_replication_SECONDARY_BINARY.csv",
    )
    pd.DataFrame([secondary_binary]).to_csv(
        secondary_binary_path,
        index=False,
    )
        ...
        transition_rows.append(
            {
                ...
                "seed": int(row["seed"]),
                "entered_regime_050": bool(
                    row["entered_regime_050"]
                ),
                "first_transition_step_050": (
                    transition_step_from_curve(
                        row["learning_curve_csv"],
                        REGIME_SUCCESS_THRESHOLD,
                    )
                ),
                "maximum_logged_validation_accuracy": float(
                    row["maximum_logged_validation_accuracy"]
                ),
    ...
    transition_path = os.path.join(
        FINAL_DIR,
        "confirmatory_replication_TRANSITIONS.csv",
    )
    transitions.to_csv(transition_path, index=False)

    decision = {
        "primary_supported": bool(
            primary["confirmatory_primary_supported"]
        ),
        "interpretation": (
            "Supported: wider fully active heads at eight heads increase "
            "the probability of entering the successful binding regime."
            if primary["confirmatory_primary_supported"]
            else
            "Not supported under the predeclared confirmatory rule. "
            "Do not launch another seed expansion or architecture grid "
            "for this mechanism."
        ),
        "next_action": (
            "Freeze this mechanism result and integrate Scripts 12–17 "
            "into the paper."
        ),
    }
    decision_path = os.path.join(
        FINAL_DIR,
        "confirmatory_replication_DECISION.json",
    )
    ...
    print("\nArchitecture summary:", flush=True)
    print(summary.to_string(index=False), flush=True)
    print("\nPrimary paired binary endpoint:", flush=True)
    print(primary_frame.to_string(index=False), flush=True)
    print("\nSecondary continuous outcomes:", flush=True)
    print(continuous.to_string(index=False), flush=True)
    print("\nSecondary stringent binary endpoint:", flush=True)
    print(
        pd.DataFrame([secondary_binary]).to_string(index=False),
        flush=True,
    )
...
def parent_main() -> None:
    ...
    print(f"NEW CONFIRMATORY SEEDS: {SEEDS}", flush=True)
    print(f"ARCHITECTURES: {[a['architecture_id'] for a in ARCHITECTURES]}", flush=True)
    print(f"MAX STEPS: {MAX_STEPS}", flush=True)
    print(
        "PRIMARY ENDPOINT: maximum logged validation accuracy >= 0.50",
        flush=True,
    )
    ...
    print(
        f"[MATCHED PARAMETER COUNT] {matched_parameter_count:,}",
        flush=True,
    )
...
if __name__ == "__main__":
    ...
    if arguments.worker:
        if not arguments.architectures:
            raise ValueError(
                "--architectures is required in worker mode."
            )
